...
class CKFinder
{
	...
	public function SetupFCKeditorObject( &$editorObj, $imageType = null, $flashType = null )
	{
		...
		$editorObj->Config['LinkUploadURL'] = $dir . urlencode( 'core/connector/php/connector.php?command=QuickUpload&678&type=Files' ) ;
		$editorObj->Config['ImageUploadURL'] = $dir . urlencode( 'core/connector/php/connector.php?command=QuickUpload&234&type=') . ( empty( $imageType ) ? 'Images' : $imageType ) ;
		$editorObj->Config['FlashUploadURL'] = $dir . urlencode( 'core/connector/php/connector.php?command=QuickUpload&456&type=') . ( empty( $flashType ) ? 'Flash' : $flashType ) ;
	}
